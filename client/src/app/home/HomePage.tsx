...
const HomePage = (props: Props) => {
  ...
  async function getMovieByTitleFn() {
    ...
    if (response.Response == "False") {
      ...
      dispatch(
        setAlert({
          content: "Server Error! Please try agin later",
          type: "error",
        })
      );
    }
    ...
    <div>
      ...
      >
        <p className="text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-bold">
          Welcome to <span style={{ color: colors.red }}>Watchlists</span>
        </p>
        <p className="mt-[3%]">Search movies, add them to your watchlist</p>
        <p className="mt-[1%]">
          By clicking on <TurnedInNotIcon />, the movie will be added to your
          watchlist
        </p>
      </div>
      <div className="flex">
      ...
      <div className="mt-[3%]">
        ...
        {movieNotFound && (
          <div className="text-[30px] text-center">Movie Not Found</div>
        )}
      </div>
